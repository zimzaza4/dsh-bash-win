...
Windows 的沙箱执行器(`dsh-bash-sandbox` / `dsh-pwsh-sandbox`)用**受限令牌**
启动子进程,而 Git Bash(Cygwin/MSYS2)在受限令牌下**无法初始化**
(`CreateFileMapping` 拒绝访问,进程直接 fatal error)。因此:

- DSH 在 Windows 上**禁用了官方 bash 工具**(只有 pwsh),Windows 用户没有 bash 可用;
- 即使强行接入,官方沙箱对 Cygwin(起不来)和 WSL(VM 内文件系统不受 Windows
  ACL 约束)都无效。

本插件通过 `ctx.subprocess` 直接 spawn `bash.exe` / `wsl.exe`,绕开受限令牌层,
把 Git Bash 和 WSL2 带进 DSH,并**自建有效沙箱**(WSL 内 bwrap)与**审批把关**。
...
```powershell
cd D:\Develop\ToolDev\dsh-bash-win
node --check lib/index.js
node --check lib/client.js
npm publish   # publishConfig.access 已设为 public,直接发布
```
