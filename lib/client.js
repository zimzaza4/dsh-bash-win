window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function TerminalRow(props) {
			...
			if (isRecord) {
				const argsRaw = settled ? (block.call && typeof block.call === 'object' && typeof block.call.arguments === 'string' ? block.call.arguments : undefined) : (typeof block.arguments === 'string' ? block.arguments : undefined);
				if (typeof argsRaw === 'string') {
					try {
						const parsed = JSON.parse(argsRaw);
						if (parsed && typeof parsed === 'object' && typeof parsed.command === 'string') fallbackCommand = parsed.command;
					} catch (e) { /* unparsable args */ }
				}
			}
			...
			if (resultView !== null) {
				...
				// No terminal views (e.g. replayed history): still show the
				// command, plus settled text output when present.
				let outputText = '';
